...
func TestCrushIgnore(t *testing.T) {
	...
	// Create a .recurseignore file that ignores .log files
	require.NoError(t, os.WriteFile(".recurseignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .recurseignore file
	crushignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".recurseignore"), []byte(crushignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .recurseignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .recurseignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .recurseignore in subdir that ignores normal_nested
	subCrushignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".recurseignore"), []byte(subCrushignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .recurseignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .recurseignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .recurseignore")
