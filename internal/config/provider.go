...
import (
	...
	"github.com/rand/recurse/internal/agent/hyper"
...
func cachePathFor(name string) string {
	...
	// for windows, it should be in `%LOCALAPPDATA%/recurse/`
	// for linux and macOS, it should be in `$HOME/.local/share/recurse/`
	...
	providerOnce.Do(func() {
		...
		wg.Go(func() {
			...
			if err != nil {
				...
				errs = append(errs, fmt.Errorf("Crush was unable to fetch an updated list of providers from %s. Consider setting CRUSH_DISABLE_PROVIDER_AUTO_UPDATE=1 to use the embedded providers bundled at the time of this Crush release. You can also update providers manually. For more info see recurse update-providers --help.\n\nCause: %w", catwalkURL, providerErr)) //nolint:staticcheck
				return
			}
