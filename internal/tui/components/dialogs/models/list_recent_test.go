...
func TestModelList_RecentlyUsedSectionAndPrunesInvalid(t *testing.T) {
	...
	confPath := filepath.Join(cfgDir, "recurse", "recurse.json")
	...
	dataConfDir := filepath.Join(dataDir, "recurse")
	...
	origConfPath := filepath.Join(cfgDir, "recurse", "recurse.json")
	...
	dataConf := filepath.Join(dataDir, "recurse", "recurse.json")
