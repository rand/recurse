// Package logo renders a Recurse wordmark in a stylized way.
package logo
...
// Opts are the options for rendering the Recurse title art.
type Opts struct {
	FieldColor   color.Color // diagonal lines
	TitleColorA  color.Color // left gradient ramp point
	TitleColorB  color.Color // right gradient ramp point
	BrandColor   color.Color // brand text color
	VersionColor color.Color // Version text color
	Width        int         // width of the rendered logo, used for truncation
}

// Render renders the Recurse logo. Set the argument to true to render the narrow
...
// SmallRender renders a smaller version of the Recurse logo, suitable for
// smaller windows or sidebar usage.
func SmallRender(width int) string {
	t := styles.CurrentTheme()
	title := styles.ApplyBoldForegroundGrad("RECURSE", t.Secondary, t.Primary)
...
func letterE(stretch bool) string {
	...
	return joinLetterform(
		left,
		stretchLetterformPart(bar, letterformProps{
			stretch:    stretch,
			width:      4,
			minStretch: 7,
			maxStretch: 12,
